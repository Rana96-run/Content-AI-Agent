...
function listen-slides {
    Write-Host "Generating listening slides..." -ForegroundColor Cyan
    (Invoke-WebRequest -Uri "$BASE/api/agent/listening/slides" -Method POST -UseBasicParsing -TimeoutSec 120).Content | ConvertFrom-Json | ConvertTo-Json -Depth 5
}
...
Write-Host "listen          listen-status    listen-reset    listen-slides" -ForegroundColor Yellow
